...
- Coupling structured baseline: the baseline split weight is built from Green-response L2 norms, using `R_x^(j) = ||G_x[f(., y_j)]||`, `R_y^(i) = ||G_y[f(x_i, .)]||`, and `w_ij = R_x^(j) / (R_x^(j) + R_y^(i) + eps)`. This keeps operator information in the baseline through the pretrained Green responses instead of a separate hand-coded coefficient split.
