...
- CouplingNet optional five-stencil coupler: set `coupling_model.coupler.enabled=true` to apply a projection-preserving local 2D five-stencil StencilMLP after the fixed balance projection. The projection first enforces `phi + psi = f`; the coupler then gathers center, `i+/-1`, and `j+/-1` features from the post-projection difference `diff = 0.5 * (phi - psi)`, the pre-projection balance residual `r0 = f - (phi_raw + psi_raw)` as an auxiliary diagnostic channel, normalized source `f`, and coefficient channels, then applies `(phi, psi) -> (phi + delta, psi - delta)` so `phi + psi = f` remains preserved after the coupler.
...
- Coupling losses: the model returns final axial flux-divergences after the fixed balance projection and optional five-stencil coupler. Coupling training now uses three independently controlled losses under `coupling_training.losses`: represented-solution L2 consistency, energy consistency, and cross consistency. The nested loss config is the only supported schema.
- Coupling hybrid-detach training: set `coupling_training.hybrid_detach.enabled=true` to supervise both the projected split and the coupled split. The projected energy term trains the main CouplingNet, while the coupled energy term trains the five-stencil coupler using detached projected fields and detached pre-projection residuals when `coupling_training.hybrid_detach.detach_coupler_input=true`.
- Coupling Stage 2 coupler-only training: set `coupling_training.stage2.enabled=true` with `coupling_training.stage2.checkpoint_path` to load a Stage 1 checkpoint, freeze the main CouplingNet, and optimize only the post-projection five-stencil coupler using coupled energy consistency. Projected energy is logged as a diagnostic baseline together with relative energy improvement and delta norm ratio. Delta regularization and early stopping are not used; best-checkpoint selection remains based on validation `rel_sol`.
