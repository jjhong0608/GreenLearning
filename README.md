...
- Complex coefficient normalization: each physical segment is mapped to the unit interval before GreenNet and Green reconstruction. The unit coefficients are `a_unit=a_phys`, `ap_unit=L*ap_phys`, `b_unit=L*b_phys`, `c_unit=L^2*c_phys`, and the Green reconstruction source is `f_unit=L^2*f_phys`. GreenNet trunk coordinates are always unit `(t, eta) in [0,1]^2`; unit reconstruction integrates `G_unit(t, eta) f_unit(eta)` over `eta` without multiplying by an additional segment length. Complex CouplingNet reuses the unit-scaled coefficient branches but intentionally keeps its mandatory source branch in physical source space.
- Complex CouplingNet behavior: the model is source-conditioned like the unit-square CouplingNet. Full-grid `rhs` is gathered to valid points, lifted into segment-local physical source profiles with endpoint hard-zero values, and normalized by `A=sqrt(integral_0^1 f_phys(s(t))^2 dt)`. The normalized profile enters the mandatory source branch and the two directional outputs are multiplied by `A`, so the model returns raw physical `[phi_raw, psi_raw]` with shape `(B,2,P)`. The coefficient branch is controlled by `coupling_model.coefficient_terms` in active `[a,b_primary,b_transverse,c]` order: when `convection=true`, x/Phi segments receive `[L_x*b_x, L_x*b_y]` and y/Psi segments receive `[L_y*b_y, L_y*b_x]`; `a'` and transverse convection are not added to GreenONet reconstruction branches, which keep `[a,ap,b_primary,c]`. The geometry branch consumes `[s_left, s_right, s_mid, L, L^2, 1/L]`, the fixed-line transverse branch consumes globally normalized transverse Fourier features, and the primary trunk consumes segment-local `t`. Set `coupling_model.axis_1d_trunk.transverse_trunk.enabled=true` to add a separate pointwise transverse trunk: `phi`/x-path uses `x_local_t` as the primary coordinate and `y_local_t` as the transverse coordinate, while `psi`/y-path uses `y_local_t` as the primary coordinate and `x_local_t` as the transverse coordinate.
- Complex balance projection: complex mode requires `balance_projection={"enabled": true, "mode": "symmetric"}`. The equal-half residual correction is applied directly to the two physical raw outputs, preserving their difference while enforcing `phi+psi=rhs`. Only after projection are the Green reconstruction sources formed as `Phi_unit=L_x^2*phi` and `Psi_unit=L_y^2*psi`. `smooth_mask` remains unit-square-only, and the retired `geometry_weighted` mode is rejected with a retraining error. Complex CouplingNet output contract version 2 records these physical-raw semantics in checkpoints; legacy unversioned raw-unit checkpoints cannot be loaded into the new model.
...
- CouplingNet optional shared axis-1D trunk: set `coupling_model.axis_1d_trunk={"enabled": true, "boundary_aware_modes": k}` to use one shared 1D trunk for both axes in unit-square mode. In this mode `phi` evaluates the shared trunk on `x`, `psi` evaluates the same trunk on `y`, and a separate transverse branch receives only boundary-aware features `Enc_k(t)=[sin(n*pi*t), cos(n*pi*t)]` for `n=1..k`; raw transverse coordinate `t` is not included. For `phi`, `t` is the fixed line coordinate `y`; for `psi`, `t` is the fixed line coordinate `x`. In complex mode, the primary trunk is always segment-local 1D `t`, and `axis_1d_trunk.num_frequencies` / `max_frequency` control Fourier features of the globally normalized fixed-line transverse branch. Set `axis_1d_trunk.transverse_trunk={"enabled": true, "fusion": "product"}` to add pointwise cross-axis local coordinates through a separate trunk. The supported transverse trunk fusion modes are `"product"` and `"product_fuser"`, and `enabled=false` preserves the previous complex behavior. The default values are `boundary_aware_modes=4`, `num_frequencies=4`, `max_frequency=8.0`, and `transverse_trunk={"enabled": false, "fusion": "product"}`.
