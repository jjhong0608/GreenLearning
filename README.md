...
- Complex optional normalized post-line-search stationarity: `coupling_training.post_line_search_stationarity.enabled=true` is available only with symmetric-tangent `eta_strategy="closed_loop_exact_line_search"`. Let `S=H_x+H_y`, `A=S^T M_Omega S`, `g=S^T M_Omega m0`, and `z=D^-1 g`. The scalar line search minimizes response mismatch only on `{-eta*z}`; the additional full residual is `r_stat=g-eta_star*A*z`. The per-sample regularizer is `(r_stat^T D^-1 r_stat)/(g^T D^-1 g+eps)`, and `weight` adds its batch mean to the existing reference-free objective. It deliberately uses uncapped `eta_star` to measure tangent-line alignment while the forward projection continues to use `eta_applied=min(eta_star,eta_cap)`. `A*z` is evaluated as one extra cached segment-local adjoint action `S^T M_Omega(Sz)` with no global matrix or solve. Logs and CSV distinguish the weighted `loss_tangent_post_line_search_stationarity` from the unweighted `tangent_post_line_search_stationarity_ratio`; artifacts also store `tangent_hessian_direction`, `tangent_stationarity_residual`, and the sample ratio. The option defaults to disabled and does not change model or checkpoint tensor keys.
  ```json
  ...
  ```
  `configs/complex_coupling_soap_tangent_stationarity.json` is the paired pilot: it differs from the tangent baseline only by this stationarity objective and `best_physics_checkpoint.enabled=true`. Its current explicit weight is `1e-2`. Best-energy selection remains based on `loss_energy_optimized`; best-physics selection uses the stationarity-augmented total loss. The stationarity term measures the learned Green-response surrogate and therefore complements rather than replaces canonical energy.
- Complex optional response-trust loss: `coupling_training.response_trust.enabled=true` is available only with symmetric-tangent `eta_strategy="closed_loop_exact_line_search"` and is mutually exclusive with the optimized post-line-search stationarity loss. Let `m_pre=H_x*p_tilde-H_y*q_tilde`, let the production correction use the actual capped `delta=-eta_applied*D^-1*g`, and let `m_post=m_pre+(H_x+H_y)*delta`. The fixed source-response normalization is `E_f=||H_x(f/2)||_M^2+||H_y(f/2)||_M^2`. Per sample, response trust evaluates `||m_post||_M^2/(E_f+eps) + trust_weight*||(H_x+H_y)delta||_M^2/(E_f+eps)`, with `trust_weight=0.01` by default. This directly penalizes the applied post-correction mismatch and discourages a large response correction without reading `sol` or target `phi/psi`. It reuses `m_post-m_pre` for the correction response and adds only one matrix-free `forward_pair` for `H_x(f/2),H_y(f/2)`. The existing uncapped stationarity ratio is still computed with one adjoint action as a diagnostic, but `loss_tangent_post_line_search_stationarity` is absent in response-trust mode. Logs and CSV report the weighted `loss_tangent_response_trust`, the unweighted combined ratio, its post-mismatch/correction components, source-response energy, and the diagnostic stationarity ratio. Artifact summary and selected raw arrays record the same provenance and response fields.
  ```json
  ...
  ```
  The block defaults to disabled, so existing training and checkpoint tensors are unchanged. A conservative first paired ablation can use `weight=1e-3`; this is an experiment setting rather than the dataclass default. Best-energy selection remains on `loss_energy_optimized`, while best-physics selection uses the response-trust-augmented total loss.
  ```json
  "balance_projection": {
    "enabled": true,
...
- Complex total objective: `loss = loss_split_objective + loss_weak_operator_closure + one_of(loss_tangent_post_line_search_stationarity, loss_tangent_response_trust)`, omitting each disabled optional term. Response trust and optimized stationarity are mutually exclusive, although response-trust runs retain the unweighted stationarity ratio as a diagnostic. With relative split disabled, the split objective is `loss_energy_optimized`. The trainer and evaluator record the fixed `boundary_weight`, optimized energy, unweighted canonical energy, bulk, boundary total/x/y, and any enabled optional metrics. Boundary diagnostics remain available when the boundary weight is zero; transition-specific metrics are not generated.
