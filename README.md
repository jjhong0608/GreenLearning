...
- CouplingNet optional trunk positional encoding: set `coupling_model.trunk_positional_encoding.enabled=true` to replace raw trunk coordinates with deterministic axis-aligned Fourier features `[x, y, sin(2*pi*f*x), cos(2*pi*f*x), sin(2*pi*f*y), cos(2*pi*f*y)]`. Frequencies are log-spaced from `1` to `max_frequency`; the defaults `num_frequencies=4` and `max_frequency=8.0` give `[1, 2, 4, 8]`. Set `include_input=false` to drop raw `(x, y)` from the encoded trunk input. This feature is independent of `green_response_feature` and source stencil lift.
- Balance projection: CouplingNet defaults to `coupling_model.balance_projection="symmetric"`, the fixed interior `0.5/0.5` residual split. Set `balance_projection="smooth_mask"` to use smooth transverse masks that preserve exact interior `phi + psi = f` while damping the raw difference mode near transverse boundaries. For `smooth_mask`, `coupling_model.smooth_mask_power` applies the fixed mask softening exponent `p` to `m_phi` and `m_psi`, and `coupling_model.smooth_mask_diff_power` applies the difference-mode exponent `q` in `beta = 0.5 * (2 * alpha_soft)^q`; both default to `1.0`, which recovers the previous projection. Set `coupling_model.smooth_mask_diff_power_trainable=true` to learn only `q` as a bounded scalar with `smooth_mask_diff_power_min <= q <= smooth_mask_diff_power_max` (defaults `0.25` and `2.0`); the raw parameter is optimized in its own AdamW group with the main learning rate and `weight_decay=0.0`, and no extra q regularization loss is added. The projection always uses the physical `rhs_raw`; source lift, losses, and evaluation targets remain unchanged.
...
- Coefficients: the training CLI resolves `a_fun`, `apx_fun`, `apy_fun`, plus `b_fun` and `c_fun`, then forwards the same function set into the axial sampler and Coupling datasets.
- Coefficient functions: set `dataset.coefficient_functions_path` to a Python file that defines callable `a_fun`, `apx_fun`, `apy_fun`, `b_fun`, and `c_fun` with signature `(x, y) -> Tensor`; `configs/sinusoidal_coefficients.py` is a default-equivalent example. Omit it or set it to `null` to use the built-in defaults in `greenonet.coefficients`; invalid custom files fail fast instead of falling back silently.
