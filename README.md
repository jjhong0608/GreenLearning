...
- CouplingNet: an axial branch/trunk MIONet consumes `(a, b, c, f)` together with interior coordinates, builds an inverse structured Green-response baseline `phi_str, psi_str`, predicts learned corrections `Delta_phi, Delta_psi`, projects only the residual `(1 - lambda)f` balance target, and then adds back the `lambda`-scaled structured baseline.
...
- Coupling structured baseline: the baseline split weight is built from Green-response L2 norms, using `R_x^(j) = ||G_x[f(., y_j)]||`, `R_y^(i) = ||G_y[f(x_i, .)]||`, and the opposite-side rule `w_ij = R_y^(i) / (R_x^(j) + R_y^(i) + eps)`. A single learnable global scalar `lambda = sigmoid(lambda_logit)` scales how much of `f` is assigned to that structured baseline.
...
- Visualizing the exact structured baseline on axial lines: `python visualize_structured_baseline.py --npz-path path/to/sample.npz --outdir structured_baseline_plots --step-size 0.5 --n-points-per-line 3 --integration-rule trapezoid`. The script reuses the current coefficient `a_fun` from `cli/train.py`, builds exact Green responses with `greenonet.greens.ExactGreenFunction`, and saves Plotly figures for the interior RHS, exact flux-divergences, structured baselines, and `correction = exact flux-divergence - structured baseline` on each x- and y-axial line, plus summary heatmaps for `w`, `phi_str`, and `psi_str`.
