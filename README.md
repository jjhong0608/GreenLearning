...
- CouplingNet: an axial branch/trunk MIONet consumes `(a, b, c, f)` together with interior coordinates, builds a structured Green-response baseline `phi_str, psi_str`, predicts learned corrections `Delta_phi, Delta_psi`, and applies the same balance projection after assembling the full flux-divergence fields.
...
- Coupling structured baseline: the baseline split weight is built from Green-response L2 norms, using `R_x^(j) = ||G_x[f(., y_j)]||`, `R_y^(i) = ||G_y[f(x_i, .)]||`, and the opposite-side rule `w_ij = R_y^(i) / (R_x^(j) + R_y^(i) + eps)`. This keeps operator information in the baseline through the pretrained Green responses instead of a separate hand-coded coefficient split.
- Coupling learned correction path: the neural branch path now fuses all line-wise inputs `(a, b, c, f)` through separate encoders and a learned projection, so the correction term stays coefficient-aware and source-aware.
- Coupling losses: the model still returns projected axial flux-divergences after the fixed balance projection. Coupling training still uses the represented-solution consistency workflow, plus optional flux consistency when `coupling_training.flux_consistency_enabled=true`.
