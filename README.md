...
- Coupling loss config: use `coupling_training.losses.l2_consistency`, `coupling_training.losses.flux_consistency`, and `coupling_training.losses.cross_consistency`, each with `enabled`, `weight`, and `weight_mode`. Supported modes are `manual` and `auto_operator`, with `l2_consistency.weight_mode` fixed to `manual`. The sample config keeps `l2_consistency` enabled and the two auxiliary losses disabled by default.
