...
- Disk variable-diffusion objective study: `numerical_examples/disk/coefficient.py` defines the fixed asymmetric coefficient `a(x,y)=1+0.5 sin(2 pi x) sin(4 pi y)` on the radius-0.5 disk, with its analytic derivatives and zero convection/reaction. The coefficient remains uniformly elliptic with `0.5 <= a <= 1.5`. Both GreenNet and CouplingNet use `data/geometry/disk_radius_05_1_128.npz`; Disk CouplingNet configs reuse the experiment-local GreenNet checkpoint `checkpoints/numerical_examples/disk/green/model.safetensors` and enable only the diffusion coefficient branch.
- Disk objective matrix: `numerical_examples/disk/disk_{energy_only,energy_response_trust,energy_stationarity,energy_response_trust_stationarity}_seed{0,1,2,3}.json` defines the fixed `2 x 2 x 4` comparison of canonical energy, response trust, and source-normalized stationarity. All 16 runs share `num_train=4800`, `num_valid=300`, batch size 200, 2,400 optimizer steps, `warmup_steps=240`, `validation_every_steps=24`, `product_fuser`, K=4 tangent correction, local weak-residual reconstruction, boundary-off canonical energy, and the same 50-sample reference test set. Indexed-GP and CouplingNet seeds are paired at `0/1/2/3`; only the two auxiliary-loss activation flags vary across objective cells. `numerical_examples/disk/base_config.json` is the seed-0 joint-objective template.
...
- Sequential Disk queue: `cli/run_disk_experiment_queue.py` runs the 16 variable-diffusion objective/seed configs in the fixed order `energy_only`, `energy_response_trust`, `energy_stationarity`, and `energy_response_trust_stationarity`, with seeds `0..3` inside each objective. Its default output root is `checkpoints/numerical_examples/disk`, so run directories are named `coupling_<objective>_seed<seed>` beside the existing `green` directory. It shares the Unit-square queue's `queue.log`, atomic `queue_status.json`, PID guard, per-run `popen_stdout_stderr.log`, `_SUCCESS` restart skip, first-failure stop, incomplete-directory refusal, and `--max-cpu-threads` behavior. Run it with `PYTHONPATH=src ~/.conda/envs/green_net/bin/python cli/run_disk_experiment_queue.py --max-cpu-threads 4`.
- Split Disk queues: `cli/run_disk_experiment_queue_seed01.py` runs all four objectives for seeds `0,1`, and `cli/run_disk_experiment_queue_seed23.py` runs the same objectives for seeds `2,3`; each file therefore owns exactly eight disjoint configs. Both retain the common `checkpoints/numerical_examples/disk` output root and existing `coupling_<objective>_seed<seed>` run names, but use independent `queue_seed01.*` and `queue_seed23.*` log/status/PID files so they can run concurrently without supervisor-state collisions. Launch each with the same options as the full queue, for example `PYTHONPATH=src ~/.conda/envs/green_net/bin/python cli/run_disk_experiment_queue_seed01.py --max-cpu-threads 4` and the corresponding `seed23` command.
