...
- Config: the CLI writes `config_used.json` in the `work-dir`. GreenNet runs materialize the resolved `training.optimizer` block, complex CouplingNet runs materialize the resolved Coupling optimizer block, and every active stage records configured-only scheduler provenance. DataLoader-dependent `steps_per_epoch`, total optimizer steps, and effective warmup steps are resolved later by the trainer and written to runtime provenance.
...
- First-stage schedule and validation: GreenNet and CouplingNet advance linear-warmup/cosine LR schedules after every successful cumulative optimizer call, including SOAP's first preconditioner-initialization no-op. Prefer explicit `warmup_steps`; legacy `warmup_epochs` is converted at runtime to `warmup_epochs * steps_per_epoch`, and configuring both is rejected. Active validation requires positive `validation_every_steps`, runs at interval multiples plus one non-duplicated final optimizer step, and writes independent step rows beside epoch-aggregate train rows. Coupling periodic checkpoints remain epoch-based, and GreenNet LBFGS remains scheduler-free with its existing epoch diagnostics.
...
- Unit-square Poisson source-count study: `numerical_examples/unit_square/unit_square_train{600,1200,2400,4800}_seed{0,1,2,3}.json` defines the fixed-`product_fuser` `4 x 4` experiment matrix. Every run uses 2,400 optimizer calls, `warmup_steps=240`, `validation_every_steps=24`, 300 fixed-size validation sources, and the same seed value for indexed-GP source identity and CouplingNet initialization. Because the operator is fixed Pure Poisson with `a=1`, `b=0`, and `c=0`, all three `coupling_model.coefficient_terms` flags are `false`; coefficients still enter Green reconstruction through the fixed GreenNet/operator context, but they are not variable CouplingNet branch inputs.
- Unit-square Poisson source-count result: all 16 runs completed with the same 2,400-step budget and a common 100-sample test set. Best-energy-checkpoint artifacts give seed-mean `rel_sol` values of `0.425477%`, `0.393118%`, `0.369595%`, and `0.350500%` for 600, 1,200, 2,400, and 4,800 training sources, respectively; all four paired seeds improve at every doubling. The 2,400-source result remains `5.448%` worse than the observed 4,800-source best, just outside the analysis's 5% saturation tolerance, while 4,800 also has the lowest tail error and optimized energy. Later numerical examples therefore use `num_train=4800`, `num_valid=300`, batch size 200, 2,400 optimizer steps, `warmup_steps=240`, and `validation_every_steps=24`. Reproduce the analysis with `PYTHONPATH=src ~/.conda/envs/green_net/bin/python cli/analyze_unit_square_training_size.py`; the generated report, CSVs, JSON, and Plotly figures are under `checkpoints/numerical_examples/unit_square_poisson/training_size_analysis/`.
- Disk variable-diffusion transverse-frequency study: `numerical_examples/disk_old/coefficient.py` defines the fixed asymmetric coefficient `a(x,y)=1+0.5 sin(2 pi x) sin(4 pi y)` on the radius-0.5 disk, with analytic derivatives and zero convection/reaction. The coefficient remains uniformly elliptic with `0.5 <= a <= 1.5`. Both GreenNet and CouplingNet use `data/geometry/disk_radius_05_1_128.npz`; the new CouplingNet configs reuse `checkpoints/numerical_examples/disk_diffusion/green/model.safetensors` and enable only the diffusion coefficient branch.
- Disk transverse-frequency matrix: `numerical_examples/disk/disk_transverse_f{1,2,3}_seed{0,1,2,3}.json` supplies the 12 runs that still need training. F1, F2, and F3 use `(num_frequencies,max_frequency)=(1,1.0),(2,2.0),(3,4.0)`, yielding transverse frequency sets `[1]`, `[1,2]`, and `[1,2,4]`. F4, `[1,2,4,8]`, is not regenerated: its four paired-seed results are the completed `checkpoints/numerical_examples/disk_diffusion/coupling/coupling_energy_only_seed{0,1,2,3}` runs. All 16 comparison cells use canonical energy only, `num_train=4800`, `num_valid=300`, batch size 200, 2,400 optimizer steps, `warmup_steps=240`, `validation_every_steps=24`, `product_fuser`, K=4 tangent correction, local weak-residual reconstruction, boundary-off canonical energy, and paired indexed-GP/CouplingNet seeds `0..3`.
- Archived Disk objective study: the previous canonical-energy/response-trust/stationarity `2 x 2 x 4` configs, coefficient file, and base template are retained under `numerical_examples/disk_old/`. They are historical inputs and are not part of the current transverse-frequency experiment matrix.
- Pentagram tangent-reach paper matrix: `numerical_examples/pentagram/{nvidia_a40,mac_studio}/seed*/pentagram_k*_seed*.json` contains the 32 paired configs for `K={0,1,2,3,4,5,9,10}` and seeds `0..3`. Seeds `0,2` run on `cuda:1` under `nvidia_a40/`; seeds `1,3` run on CPU under `mac_studio/`. Every run fixes the C-trunk architecture, 4,800/300 indexed-GP sources, batch size 200, 100 epochs, 2,400 optimizer calls, 240 warmup steps, validation every 24 calls, canonical-energy-only training, SOAP, deterministic algorithms, and best-energy artifact export. K=0 is physical symmetric; K=1 uses uncapped sample-wise exact line search; K>=2 uses the uncapped nested tangent subspace. `CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=50` is a launcher environment setting for the two concurrent NVIDIA workers and is intentionally not stored in JSON.
- Sequential Unit-square queue: `cli/run_unit_square_experiment_queue.py` runs the 16 source-count/seed configs one at a time, writes `queue.log`, `queue_status.json`, and `queue.pid`, gives each run an independent `popen_stdout_stderr.log`, skips `_SUCCESS` runs on restart, and stops at the first failed or incomplete run directory. Launch it as a detached process with `nohup setsid ... > launcher.log 2>&1 < /dev/null &`; the CLI itself contains no nested `python -c` quoting.
- Legacy Disk queues: `cli/run_disk_experiment_queue.py`, `cli/run_disk_experiment_queue_seed01.py`, and `cli/run_disk_experiment_queue_seed23.py` enumerate the archived objective-study names. They are not launchers for the current transverse-frequency matrix; the 12 new configs are intentionally the only new experiment files in this change.
...
- Complex CouplingNet behavior: output contract v6 is source-conditioned and response-valued. Full-grid `rhs` is gathered to valid points, lifted into segment-local physical source profiles with endpoint hard-zero values, and normalized by the length-independent amplitude `A=sqrt(integral_0^1 f_phys(s(t))^2 dt)`. The model returns raw directional reference responses `[P,Q]` with shape `(B,2,P)`, using deterministic output scales `P=L_x^2*A_x*P_tilde` and `Q=L_y^2*A_y*Q_tilde`. The coefficient branch is controlled by `coupling_model.coefficient_terms` in active `[a,b_primary,b_transverse,c]` order: when `convection=true`, x/Phi segments receive `[L_x*b_x, L_x*b_y]` and y/Psi segments receive `[L_y*b_y, L_y*b_x]`; Green reconstruction branches remain `[a,ap,b_primary,c]`. The optional geometry branch consumes `[s_left,s_right,s_mid,L,L^2,1/L]`, the optional fixed-line transverse branch consumes globally normalized transverse Fourier features, and the mandatory primary trunk consumes segment-local `t`.
- Complex pointwise length context: when `axis_1d_trunk.transverse_trunk.enabled=true`, `length_context=true` is required and one shared four-input transverse MLP receives `[t_perp, log(L_perp/L_ref), log(L_parallel/L_perp), kappa]`, where `kappa=4*L_parallel^2*L_perp^2/(L_parallel^2+L_perp^2)^2` and `L_ref` is the larger global geometry extent. The x/Phi path uses `(L_parallel,L_perp,t_perp)=(L_x,L_y,y_local_t)`; the y/Psi path swaps the axes. If disabled, the primary trunk is used directly and neither the transverse trunk nor its fuser is instantiated.
...
- Complex optional symmetric-tangent Green-response projection: set `mode="symmetric_tangent_green_response"` to start from the exact-balanced symmetric pair `p_tilde=(rhs+p-q)/2`, `q_tilde=(rhs-p+q)/2`. With `m0=H_x p_tilde-H_y q_tilde`, the reference-free gradient is `g=(H_x+H_y)^T M_Omega m0`, and the cached Jacobi denominator is `D=gamma_x^2+gamma_y^2+(relative_lambda+denominator_relative_eps)*mean(gamma_x^2+gamma_y^2)`. `subspace_dimension` defaults to `1`, preserving the existing fixed or closed-loop scalar line-search path exactly. The backward-compatible `eta_strategy="fixed"` uses `delta=-eta*g/D`; `eta_strategy="closed_loop_exact_line_search"` forms `z=g/D`, `v=(H_x+H_y)z`, and computes sample-wise `eta_star=(g^T z)/(<v,v>_M+eps_sample)`. The default `eta_cap_enabled=true` applies `eta_applied=min(eta_star,eta_cap)` and retains the existing half-cosine cap warmup. Set `eta_cap_enabled=false` only with closed-loop K=1 to apply `eta_applied=eta_star`; no eta-cap schedule is then created, and logs/artifacts explicitly record the uncapped policy. Here `eta` is the final K=1 safety cap only when the cap is enabled and is never a learned parameter. All tangent paths use `phi=p_tilde+delta`, `psi=q_tilde-delta`, preserve `phi+psi=rhs`, and use no reference target, row norm, global response matrix, full-Gram matrix, or linear solve. Frozen segment-local response blocks are built once and reused. For fixed K=1, `eta=0` is exactly the physical-symmetric ablation.
...
- The `separable` label applies only to the pointwise Jacobi denominator `D`: it stores no off-diagonal x/y correlation. The tangent gradient and every nested direction nevertheless use the full matrix-free action `A=(H_x+H_y)^T M_Omega(H_x+H_y)`, including `H_x^T M H_y` and its transpose. Thus K=1 already aggregates mismatch information along the horizontal and vertical segments through each point, and K>=2 repeatedly applies `D^-1 A`, propagating influence through longer chains of intersecting axial segments. Two source points need not share one axial segment to couple: it is sufficient that one can reach an evaluation point horizontally and the other vertically. Segment breaks caused by a hole block the direct action, but an indirect chain around the hole can still connect them. Response-space Gram-Schmidt only orthogonalizes directions; it does not create this connectivity.
  ```json
  "balance_projection": {
    "enabled": true,
...
- Tangent preconditioner variants: `docs/tangent_preconditioner_variants_design.md` is the canonical contract for the implemented `separable`, `exact_diagonal`, `absolute_cross_axis`, and `normalized_quadratic_cross_axis` choices. They use `D_sep=a+b+d`, `D_exact=a+b+2c+d`, `D_abs=a+b+2*abs(c)+d`, and `D_q=a+b+4q+d`, where `q=c^2/(a+b+epsilon_q)`. The cross-axis diagonal `c=diag(H_x^T M_Omega H_y)` is accumulated from segment-local response blocks without materializing a global response or Gram matrix. Set `preconditioner_variant` under `symmetric_tangent_green_response`; omission keeps the existing `separable` result. `cli/audit_tangent_preconditioners.py` evaluates all four variants against K=1..4 from one frozen raw-output stream and one response context. Its explicit `--posthoc-tangent-override` permits a `physical_symmetric` checkpoint to be screened with recorded audit-only closed-loop settings; without this flag the CLI remains strict and rejects non-tangent training configs. Intermediate strength parameters and ReLU/positive-part variants remain excluded.
- Tangent preconditioner frozen audit: the completed 100-sample Pentagram `coupling11` audit is under `checkpoints/pentagram/coupling11/tangent_preconditioner_4x4_audit/`. `separable K=4` produced the lowest mean optimized energy (`1.741050e-5`) and `rel_sol` (`1.112221e-2`). Increasing separable K from 1 to 4 reduced response mismatch by `96.182%` and `rel_sol` by `81.248%`, while changing the K=4 preconditioner produced only small differences. The frozen evidence therefore retains `separable` as the default and does not replace paired retraining.
- Tangent preconditioner training-bias control: `checkpoints/annulus_CDR/tangent_preconditioner_training_bias_comparison/` applies one shared schema-v2 response context and one common post-hoc audit contract to non-tangent `coupling5`, separable fixed-eta `coupling8`, and separable line-search `coupling9`. On the non-tangent checkpoint, exact versus separable changed K=4 `rel_sol_equal_mean` by only `-0.269%`; the corresponding changes were `-0.369%` and `-0.393%` on the tangent-trained checkpoints. The common operator has mean `rho=0.047764`, and the exact cross term changes the damped separable denominator by only `2.726%` on average. This rules out separable-specific training adaptation as the primary explanation for the near-equality. Tangent training does adapt proposals to correction as a whole, while increasing K remains the dominant effect. The integrated report, paired 50-sample CSV, JSON summary, and Plotly comparison are stored in that directory; `separable` remains the default.
- Matrix-free cross-axis audit: `cli/audit_tangent_cross_axis_coupling.py` estimates `||C||_F^2` for `C=H_x^T M_Omega H_y` with deterministic Rademacher probes and evaluates `||(C+C^T)z_b||/||Az_b||` on frozen `z_b=D^-1 g_b` directions. It uses only segment-local `H/H^T` actions, never assembles a global Gram matrix, never solves a global system, and never reads reference `sol/phi/psi`. The Annulus CDR comparison in `checkpoints/annulus_CDR/tangent_cross_axis_coupling_comparison/` gives `R_off,C=0.998180` from 256 probes and `||C+C^T||_F/||A||_F=0.078826`. On `coupling5/8/9`, mean actual-direction cross-action ratios are `0.273876/0.259521/0.256723`, while `2*diag(C)z` accounts for only `0.126%/0.112%/0.115%` of the full cross-action norm. Thus the small exact-diagonal effect means `diag(C)` is small, not that full cross-axis coupling is absent. Jacobi does not create the small diagonal; it omits the dominant nonlocal part, which K>=2 can still access through full matrix-free `A` actions.
- Matrix-free low-rank tangent audit: `cli/audit_tangent_low_rank.py` approximates the dominant eigenspace of `T=D^-1/2 A D^-1/2`, where `A=(H_x+H_y)^T M_Omega(H_x+H_y)`, by randomized Rayleigh-Ritz actions on the cached segment-local response operator. It compares the diagonal baseline and ranks `4,8,16,32` at every `K=1,2,3,4`, without assembling `A`, solving a global system, or changing the frozen checkpoint. The unresolved spectral complement is scaled by the next Ritz value; the unit-complement form is retained only as a diagnostic because it over-damps this operator. The canonical Annulus CDR `coupling9` result is under `checkpoints/annulus_CDR/coupling9/tangent_low_rank_k1_k4_audit/`: rank-8+K4 lowers mean `rel_sol` from `1.2942%` to `1.1655%` (`-9.945%`) and response mismatch by `25.235%`, but raises optimized energy by `2.529%` and tangent-core forward+backward time from `218.68 ms` to `495.62 ms` (`2.266x`). No rank+K<4 cell beats diagonal+K4 in both mean `rel_sol` and core runtime. Therefore diagonal+K4 remains the production default; rank-4+K4 is the smallest optional accuracy ablation, not a computational replacement for K4. Runtime figures cover only tangent correction and its backward pass, not end-to-end training.
- Tangent runtime-context persistence: `docs/tangent_response_context_checkpointing.md` specifies the implemented schema-v2 sidecar. Opt in with `coupling_training.tangent_context_checkpoint.enabled=true`; the separate `tangent_response_context.safetensors` contains frozen segment-local `H_x/H_y` blocks, indices, all four preconditioner tensors, point mass, and strict SHA-256 identity metadata, while CouplingNet checkpoints remain model-only. Training, evaluation, artifact export, and both tangent audit CLIs use the same load/build/save cache lifecycle and accept `--tangent-context` as the highest-priority path override. Missing files may be built according to policy, but existing corrupt or incompatible files fail fast and are never silently replaced. Structural identities and payload digests use exact comparison; recomputed floating-point algebraic invariants use a tight, dtype-epsilon-scaled tolerance so CUDA-built contexts remain portable to CPU safetensors validation without accepting material drift. The diagnostic `data/symmetric_tangent_green_response_fields.npz` is not a runtime sidecar.
...
- Pentagram tangent-topology evidence bundle: run
  `PYTHONPATH=src ~/.conda/envs/green_net/bin/python cli/analyze_tangent_topology.py`
  to combine the fixed geometry's axial-segment incidence graph, the trained
  `K=1..4` comparison, and the frozen `coupling9` K audit. Outputs are written
  to `checkpoints/pentagram/tangent_topology_k_analysis/` as a detailed Korean
  Markdown report, JSON/CSV/NPZ data, and ten Plotly figures in HTML, JSON, PNG,
  and PDF formats. The canonical geometry has 4,572 valid points, 142 x
  segments, 147 y segments, point-graph diameter 15, and A-graph diameter 8.
  K=4 first exposes point pairs at A-distance 3, amounting to `0.960083%` of
  ordered pairs; 18 points can reach at least half of the domain only through
  this new K=4 layer. Frozen K3-to-K4 improves mean response mismatch by
  `21.252%` and mean `rel_sol` by `9.371%`, while retrained K3-to-K4 improves
  mean `rel_sol` by `9.887%`. These results establish that genuinely K=4-only
  paths exist, but the selected-sample correlations do not support attributing
  all K4 gains to topology alone. Interpret the gain as a combination of graph
  propagation and a richer spectral approximation. K=4 gives the best measured
  accuracy; K=3 remains the isolated tangent-core cost-quality knee.
- Geometry-only tangent-dimension rule: for active-point A-distance counts, set
  `K_geometry` to the smallest K satisfying both global ordered-pair reach
  `C_global(K) >= 0.99` and the lower-5% pointwise reach quantile
  `Q_0.05(C_i(K)) >= 0.99`. Applied at `h=1/128` to the four paper geometries,
  this gives `K_square=2`, `K_disk=2`, `K_annulus=4`, and
  `K_pentagram,h=4`. Regenerating the radius-0.5 Pentagram at `2h=1/64` also
  gives `K_pentagram,2h=4`, so the refinement-stabilized choice is
  `max(K_pentagram,h,K_pentagram,2h)=4`. The geometry-family common choice is
  therefore `max(2,2,4,4)=4`; this rule uses no PDE coefficient, source,
  GreenNet/CouplingNet output, or reference solution.
- Production geometry-only K selection is opt-in and keeps explicit K as the
  backward-compatible default. The fixed lower-tail quantile is `0.05`; the two
  acceptance thresholds are independently configurable. Both explicit and
  automatic modes share `max_subspace_dimension` (default `8`), and an automatic
  result above that limit fails with its required K and reach diagnostics rather
  than being clamped.
  ```json
  "symmetric_tangent_green_response": {
  ...
  ```
  Train, evaluation, artifact export, and tangent audits resolve the same dynamic
  K path. Training materializes the selected integer and disables the automatic
  block in `config_used.json`; `tangent_subspace_dimension_provenance` preserves
  the original policy, geometry SHA-256, graph dimensions, selected/max reach,
  and setup time. The tangent response-context sidecar remains K-independent.
  Increasing K adds approximately O(K) response-operator actions and O(K^2)
  response-space modified Gram-Schmidt work; geometry selection is therefore a
  structural default, not a claim that the selected K is PDE-optimal.
- Pentagram full-reach validation: setting both reach thresholds to `1.0` and
  `max_subspace_dimension=9` resolves `K=9` because the canonical geometry has
  A-graph diameter 8. A frozen five-sample, 4,572-point CPU smoke test using the
  current GreenNet context preserved physical balance to `1.33e-15`, kept all
  nine directions active with finite backward gradients, and monotonically
  reduced the response cost. Relative to K=4, median tangent forward+backward
  time increased from `0.221 s` to `0.525 s`, and retained projection tensor
  storage increased from `10.54 MiB` to `15.77 MiB`; K=9 is supported but is a
  materially more expensive full-reach endpoint.
- Geometry-only K visualization: run
  `PYTHONPATH=src ~/.conda/envs/green_net/bin/python cli/visualize_geometry_k_connectivity.py`
  to draw K=1..4 cumulative structural reach for Square, Disk, Annulus, and
  Pentagram. Each domain figure colors the first-reach shell from a representative
  lower-5%-tail seed selected at the step immediately before `K_geometry`, while
  the combined 4x4 figure colors every point by its
  pointwise reachable-domain fraction `C_i(K)`. The figures describe the support
  patterns that successive matrix-free A actions can represent; they are not
  literal zero/nonzero maps of the generally dense production gradient `g0`.
  HTML/JSON/PNG/PDF figures, CSV/NPZ data, summary, and a Korean report are written
  to `checkpoints/geometry_k_connectivity_visualization/`.
  ```json
  "balance_projection": {
    "enabled": true,
...
- Automatic-K/artifact variant: `configs/unit_square_minimal_concat_fuser_auto_k_artifacts.json` preserves that minimal model and training protocol, enables the geometry-only `0.99/0.99` reach rule with `max_subspace_dimension=8`, and enables automatic best-energy artifact export with `data/visualization_mesh/unit_square_h_1_128_mesh.npz`. The canonical unit-square geometry resolves to `K=2`, and successful training writes artifacts to `<work-dir>/artifacts_best_energy/`.
...
- CouplingNet optional complex auxiliary networks: complex mode keeps `axis_1d_trunk.enabled=true` and the primary segment-local trunk mandatory, but independently controls the segment geometry branch with `coupling_model.geometry_branch.enabled`, the globally normalized fixed-line Fourier branch with `coupling_model.axis_1d_trunk.fixed_line_transverse_branch.enabled`, and the pointwise cross-axis trunk with `coupling_model.axis_1d_trunk.transverse_trunk.enabled`. The first two new flags default to `true`, preserving existing all-on checkpoint keys and values. A disabled pointwise trunk may use `length_context=false`; an enabled pointwise trunk requires `length_context=true`. Turning learned geometry modules off does not remove segment metadata, deterministic `A*L^2` output scaling, physical projection, tangent correction, or Green reconstruction. Full and minimal checkpoints are architecture-specific and cross-loading fails with an explicit mismatch error.
- CouplingNet optional shared axis-1D trunk: set `coupling_model.axis_1d_trunk={"enabled": true, "boundary_aware_modes": k}` to use one shared 1D trunk for both axes in unit-square mode. In this mode `phi` evaluates the shared trunk on `x`, `psi` evaluates the same trunk on `y`, and a separate transverse branch receives only boundary-aware features `Enc_k(t)=[sin(n*pi*t), cos(n*pi*t)]` for `n=1..k`; raw transverse coordinate `t` is not included. For `phi`, `t` is the fixed line coordinate `y`; for `psi`, `t` is the fixed line coordinate `x`. In complex mode, the primary trunk is always segment-local 1D `t`, while `axis_1d_trunk.num_frequencies` and `max_frequency` configure the fixed-line branch only when that auxiliary branch is enabled.
...
- Pentagram dataset/fusion screening: `numerical_examples/pentagram/pentagram_base.json` is paired with six configs named `pentagram_train{1200,2400,4800}_{product,product_fuser}.json`. They form the complete `3 x 2` experiment over fixed indexed-GP training-set size and fusion family. The `product` variants set both `coupling_model.branch_fusion.mode` and `axis_1d_trunk.transverse_trunk.fusion` to `product`; the `product_fuser` variants set both to `product_fuser`. The primary comparison fixes the optimization budget at `2400` optimizer calls: train counts `1200/2400/4800` use `400/200/100` epochs, `40/20/10` warmup epochs, and `200/100/50` periodic-checkpoint intervals, respectively. This keeps the warmup at 10% and approximately 240 optimizer calls while preserving half-run periodic checkpoints. Validation count (`300`), source seed (`0`), CouplingNet training seed (`0`), deterministic-algorithm policy, test set, optimizer, projection, objective, and all other model settings remain fixed.
...
- Future tangent-subspace saturation work is specified in
  `docs/tangent_subspace_saturation_and_adaptive_stopping.md`. The design note
  separates the current response-direction activity test from convergence,
  documents configured versus effective K and inactive-direction repetition,
  and stages frozen audit, exhausted masking, adaptive stopping, and
  evidence-gated restart/block extensions. It does not change current runtime
  behavior or configuration.
- The separate 47-slide Quarto Reveal.js deck is defined in
  `docs/meeting/tangent_subspace_connectivity/tangent_subspace_connectivity.qmd`.
  Visible slide text is English, and every slide has one detailed Korean
  speaker-note block with fragment-aligned `Click` cues.
- The deck distinguishes exact balance-plane algebra, the production
  matrix-free tangent algorithm, geometry-only structural reach, and frozen
  empirical evidence. The approved narrative and slide-level evidence contract
  are recorded in
  `docs/meeting/tangent_subspace_connectivity_slide_plan.md`.
- The added benchmark section defines Pentagram variable-coefficient CDR and
  unit-square Poisson explicitly, then shows one median-`rel_sol` test source,
  reference/predicted directional sources, reference/predicted solution, signed
  errors, and the complete 100-sample test distribution. The distribution
  contract reports `rel_sol`, `rel_flux`, canonical energy, and post/pre tangent
  response mismatch; separate `phi` and `psi` error distributions are
  intentionally excluded.
- Pentagram and unit-square test-distribution Plotly assets use the iframe
  viewport height instead of a fixed canvas height. A 72 px bottom margin,
  responsive lower-axis margins, and tighter row spacing keep the energy and
  remaining-mismatch ticks and titles visible at both QA viewports.
- Rebuild the 20 offline presentation assets with
  `PYTHONPATH=src ~/.conda/envs/green_net/bin/python docs/meeting/tangent_subspace_connectivity/build_assets.py --overwrite`.
  The builder reads frozen HTML/JSON/CSV/NPZ/PNG evidence only, runs no model
  inference, and records source and generated SHA-256 identities in
  `docs/meeting/tangent_subspace_connectivity/assets/manifest.json`. Frozen
  coefficient/source/solution mesh PNGs are embedded into self-contained static
  HTML grids so a Reveal.js deck cannot lose fields by exhausting concurrent
  WebGL contexts; quantitative distributions remain offline Plotly assets.
- Render the HTML with
  `HOME=/tmp/quarto-home-tangent ~/.local/bin/quarto render docs/meeting/tangent_subspace_connectivity/tangent_subspace_connectivity.qmd`.
  No presentation PDF is generated by this workflow.
- Run final and intermediate fragment QA with
  `node docs/meeting/tangent_subspace_connectivity/qa_reveal.js --html docs/meeting/tangent_subspace_connectivity/tangent_subspace_connectivity.html --outdir docs/meeting/tangent_subspace_connectivity/screenshots/qa`.
  The fixed viewports are `1600x900` and `1280x720`; the report also rejects
  slide overflow, incoherent overlap, Plotly/static-mesh iframe failure,
