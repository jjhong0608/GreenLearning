...
- Coupling optimizer config: `coupling_training.learning_rate` is the main CouplingNet Adam learning rate. Set `coupling_training.source_stencil_lift_learning_rate` to create a separate lower-LR optimizer group for the input-side 5-stencil source encoder; omit it or set it to `null` to keep the single shared optimizer group. The shared LR schedule applies the same multiplicative factor to both groups.
