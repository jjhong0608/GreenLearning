...
- CouplingNet: an axial branch/trunk MIONet consumes `(a, b, c, f)` together with interior coordinates, builds a local pointwise smoothed inverse structured baseline `phi_str, psi_str`, predicts learned corrections `Delta_phi, Delta_psi`, projects only the residual `(1 - lambda)f` balance target, and then adds back the `lambda`-scaled structured baseline.
- Balance projection: CouplingNet keeps the same fixed symmetric interior `0.5/0.5` projection, but it now enforces the projected raw correction part against `(1 - lambda)f` instead of the full right-hand side.
- Coupling structured baseline: the old line-norm Green-response weight has been removed. The baseline now uses local pointwise responses `u_x(x_i, y_j)` and `u_y(x_i, y_j)` on the common interior grid, forms smoothed magnitudes `A_x = sqrt(u_x^2 + delta^2)` and `A_y = sqrt(u_y^2 + delta^2)`, then uses `w_ij = A_y / (A_x + A_y)` with no extra denominator epsilon. The fixed `delta` value reuses the previous baseline epsilon constant `1e-12`. A single learnable global scalar `lambda = sigmoid(lambda_logit)` still scales how much of `f` is assigned to that structured baseline.
...
- Coupling losses: the model still returns projected axial flux-divergences after the residual projection plus `lambda`-scaled baseline assembly. Coupling training still uses the represented-solution consistency workflow, plus optional flux consistency when `coupling_training.flux_consistency_enabled=true`.
...
- Visualizing the exact structured baseline on axial lines: `python visualize_structured_baseline.py --npz-path path/to/sample.npz --outdir structured_baseline_plots --step-size 0.5 --n-points-per-line 3 --integration-rule trapezoid`. The script reuses the current coefficient `a_fun` from `cli/train.py`, builds exact local pointwise Green responses with `greenonet.greens.ExactGreenFunction`, forms the same smoothed inverse weight `w = A_y / (A_x + A_y)` with fixed `delta = 1e-12`, and saves Plotly figures for the interior RHS, exact flux-divergences, structured baselines, and `correction = exact flux-divergence - structured baseline` on each x- and y-axial line, plus summary heatmaps for `w`, `phi_str`, and `psi_str`.
