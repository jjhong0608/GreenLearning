...
- Coefficient functions: set `dataset.coefficient_functions_path` to a Python file that defines callable `a_fun`, `apx_fun`, `apy_fun`, `b_fun`, and `c_fun` with signature `(x, y) -> Tensor`. Omit it or set it to `null` to use the built-in defaults in `greenonet.coefficients`; invalid custom files fail fast instead of falling back silently.
