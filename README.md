...
- Balance projection: CouplingNet defaults to `coupling_model.balance_projection="symmetric"`, the fixed interior `0.5/0.5` residual split. Set `balance_projection="smooth_mask"` to use smooth transverse masks that preserve exact interior `phi + psi = f` while damping the raw difference mode near transverse boundaries. For `smooth_mask`, `coupling_model.smooth_mask_power` applies the mask softening exponent `p` to `m_phi` and `m_psi`, and `coupling_model.smooth_mask_diff_power` applies the difference-mode exponent `q` in `beta = 0.5 * (2 * alpha_soft)^q`; both default to `1.0`, which recovers the previous projection. The projection always uses the physical `rhs_raw`; source lift, losses, and evaluation targets remain unchanged.
