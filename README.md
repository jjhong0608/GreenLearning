...
- CouplingNet: an axial branch/trunk MIONet consumes `(a, b, c, f)` together with interior coordinates, builds a centered local structured baseline from smoothed pointwise Green responses, predicts learned corrections `Delta_phi, Delta_psi`, projects the raw learned part against the full `f` balance target, and then adds the centered structured term afterward.
- Balance projection: CouplingNet keeps the same fixed symmetric interior `0.5/0.5` projection and now again enforces the projected raw correction part directly against the full right-hand side `f`.
- Coupling structured baseline: the old line-norm Green-response weight has been removed. The baseline uses local pointwise responses `u_x(x_i, y_j)` and `u_y(x_i, y_j)` on the common interior grid, forms smoothed magnitudes `A_x = sqrt(u_x^2 + delta^2)` and `A_y = sqrt(u_y^2 + delta^2)`, then uses `w_ij = A_y / (A_x + A_y)` with no extra denominator epsilon. The fixed `delta` value reuses the previous baseline epsilon constant `1e-12`. The actual injected structured term is centered: `Delta_phi_str = (w_ij - 0.5) f_ij` and `Delta_psi_str = -(w_ij - 0.5) f_ij`, so it only changes the split asymmetry and sums to zero.
- Coupling learned correction path: the neural branch path still fuses all line-wise inputs `(a, b, c, f)` through separate encoders and a learned projection, so the correction term stays coefficient-aware and source-aware.
- Coupling losses: the model still returns projected axial flux-divergences after the full-`f` projection plus centered-baseline assembly. Coupling training still uses the represented-solution consistency workflow, plus optional flux consistency when `coupling_training.flux_consistency_enabled=true`.
...
- Visualizing the exact structured baseline on axial lines: `python visualize_structured_baseline.py --npz-path path/to/sample.npz --outdir structured_baseline_plots --step-size 0.5 --n-points-per-line 3 --integration-rule trapezoid`. The script reuses the current coefficient `a_fun` from `cli/train.py`, builds exact local pointwise Green responses with `greenonet.greens.ExactGreenFunction`, forms the same smoothed inverse weight `w = A_y / (A_x + A_y)` with fixed `delta = 1e-12`, converts it into the centered structured fields `Delta_phi_str = (w - 0.5) f` and `Delta_psi_str = -(w - 0.5) f`, and saves Plotly figures for the interior RHS, exact flux-divergences, centered structured baselines, and `correction = exact flux-divergence - centered structured baseline` on each x- and y-axial line, plus summary heatmaps for `w`, `Delta_phi_str`, and `Delta_psi_str`.
