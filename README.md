...
- CouplingNet optional source stencil lift: set `coupling_model.source_stencil_lift.enabled=true` to add an input-side learned two-channel 5-point stencil encoder. It reconstructs canonical full grids from `rhs_raw` and `a_vals`, feeds normalized source `f` stencils together with raw diffusion-coefficient `a` stencils into the encoder, optionally normalizes the scalar encoded field `g` by interior RMS, and uses interior-only `g` as the single branch input. Set `coupling_model.source_stencil_lift.encoder_type` to `"mlp"` for the default nonlinear stencil encoder or `"linear"` for a direct affine map from the 10 stencil features to `g`. `b_vals` and `c_vals` are not part of this first encoder version, and the physical `rhs_raw`, `rhs_norm`, balance projection, losses, and evaluation targets remain unchanged.
- Balance projection: CouplingNet defaults to `coupling_model.balance_projection="symmetric"`, the fixed interior `0.5/0.5` residual split. Set `balance_projection="smooth_mask"` to use smooth transverse masks that preserve exact interior `phi + psi = f` while damping the raw difference mode near transverse boundaries. The projection always uses the physical `rhs_raw`; source lift, losses, and evaluation targets remain unchanged.
