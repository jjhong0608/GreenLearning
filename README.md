...
- CouplingNet: a shared branch/trunk MIONet now consumes axial-line inputs `(a, a', b, c, f)` through separate 1D CNN line encoders for each branch input, fuses those branch latents into one shared branch representation, and keeps the existing trunk/readout/balance-projection path for axial flux-divergence prediction.
...
- Coupling line encoders: configure the branch-side CNN front-end under `coupling_model.line_encoder`. The default encoder uses channels `[line, pos, db]`, a three-layer Conv1d stack with output widths `[32, 64, 64]`, kernel size `5`, dilations `[1, 2, 4]`, mean+max pooling, and a config-driven `mlp_head` that maps pooled features back to the Coupling `hidden_dim`.
