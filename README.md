...
- Complex optional normalized post-line-search stationarity: `coupling_training.post_line_search_stationarity.enabled=true` is available only with symmetric-tangent `eta_strategy="closed_loop_exact_line_search"`. Let `S=H_x+H_y`, `A=S^T M_Omega S`, `g=S^T M_Omega m0`, and `z=D^-1 g`. The scalar line search minimizes response mismatch only on `{-eta*z}`; the additional full residual is `r_stat=g-eta_star*A*z`. The per-sample regularizer is `(r_stat^T D^-1 r_stat)/(g^T D^-1 g+eps)`, and `weight` adds its batch mean to the existing reference-free objective. It deliberately uses uncapped `eta_star` to measure tangent-line alignment while the forward projection continues to use `eta_applied=min(eta_star,eta_cap)`. `A*z` is evaluated as one extra cached segment-local adjoint action `S^T M_Omega(Sz)` with no global matrix or solve. Logs and CSV distinguish the weighted `loss_tangent_post_line_search_stationarity` from the unweighted `tangent_post_line_search_stationarity_ratio`; artifacts also store `tangent_hessian_direction`, `tangent_stationarity_residual`, and the sample ratio. The option defaults to disabled and does not change model or checkpoint tensor keys.
  ...
  ```
  `configs/complex_coupling_soap_tangent_stationarity.json` is the paired pilot: it differs from the tangent baseline only by this `weight=1e-3` objective and `best_physics_checkpoint.enabled=true`. Best-energy selection remains based on `loss_energy_optimized`; best-physics selection uses the stationarity-augmented total loss. The stationarity term measures the learned Green-response surrogate and therefore complements rather than replaces canonical energy.
  ```json
  "balance_projection": {
    "enabled": true,
...
- Complex total objective: `loss = loss_split_objective + loss_weak_operator_closure + loss_tangent_post_line_search_stationarity`, omitting each disabled optional term. With relative split disabled, the split objective is `loss_energy_optimized`. The trainer and evaluator record the fixed `boundary_weight`, optimized energy, unweighted canonical energy, bulk, boundary total/x/y, and any enabled relative-split, weak-closure, or stationarity metrics. Boundary diagnostics remain available when the boundary weight is zero; transition-specific metrics are not generated.
