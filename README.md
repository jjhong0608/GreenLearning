...
- CouplingNet: a shared branch/trunk MIONet now consumes axial-line inputs `(a, a', b, c, f)` through separate 1D CNN line encoders for each branch input, then fuses those five branch tokens with `coupling_model.fusion`. The default `linear` mode preserves the original concatenate-plus-linear fusion, while `gated_attention` applies single-head self-attention over the branch tokens followed by scalar gated attention pooling before the existing trunk/readout/balance-projection path.
- Coupling trunk Fourier embedding: configure the optional fixed-frequency trunk-only embedding under `coupling_model.trunk_fourier`. When enabled, CouplingNet augments the raw trunk coordinates with `sin(freq * coords)` and `cos(freq * coords)` for each configured frequency, while leaving the branch encoders untouched. The default frequency list is `[pi, 2*pi, 4*pi, 8*pi]`.
...
- Coupling losses: the model returns projected axial flux-divergences after the fixed balance projection. Coupling training now uses three independently controlled losses under `coupling_training.losses`: represented-solution L2 consistency, flux consistency, and cross consistency. The nested loss config is the only supported schema.
...
- Coupling line encoders: configure the branch-side CNN front-end under `coupling_model.line_encoder`. The encoder consumes `[line, pos, db]`, uses a dilated Conv1d stack followed by mean+max pooling, and finishes with a config-driven `mlp_head` that maps pooled features back to the Coupling `hidden_dim`.
- Coupling branch fusion: configure the branch-token fusion under `coupling_model.fusion`. `linear` keeps the prior `Linear(5 * hidden_dim -> hidden_dim)` path; `gated_attention` uses single-head self-attention across the five encoded branch tokens `[a, a', b, c, rhs]`, then scalar gated attention pooling and an output projection back to `hidden_dim`.
...
- Coupling loss config: use `coupling_training.losses.l2_consistency`, `coupling_training.losses.flux_consistency`, and `coupling_training.losses.cross_consistency`, each with `enabled`, `weight`, and `weight_mode`. Supported modes are `manual` and `auto_operator`, with `l2_consistency.weight_mode` fixed to `manual`. The checked-in Coupling sample keeps all three losses enabled, with `auto_operator` enabled for the two finite-difference auxiliary losses.
...
- Coupling auto weights: when `flux_consistency.weight_mode="auto_operator"` or `cross_consistency.weight_mode="auto_operator"`, the trainer ignores that term's configured `weight` and replaces it with an operator-aware effective weight computed from the actual batch spacing and coefficient magnitudes. `l2_consistency.weight` remains the fixed base reference weight; `flux_consistency` scales like a first-derivative loss and `cross_consistency` scales like the full conservative line operator.
...
- Plotting recent Adam-only Coupling logs from the current `_run_training_phase - epoch ...` format: `python plot_logs.py --logs checkpoints/test_diffusion/coupling/single_unknown/backward/training.log --outdir plots_coupling_recent`. This plots total loss, L2 consistency, flux consistency, cross consistency, `rel_flux`, and `rel_sol` from the current Coupling trainer log lines and ignores compile/checkpoint noise lines. The parser also accepts the logged effective-weight fields `w_*`, `val_w_*`, and the per-loss `mode_*` tokens.
