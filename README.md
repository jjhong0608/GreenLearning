...
- CouplingNet optional five-stencil coupler: set `coupling_model.coupler.enabled=true` to insert a projection-preserving local 2D five-stencil StencilMLP between the line-wise CouplingNet readout and the fixed balance projection. The coupler gathers center, `i+/-1`, and `j+/-1` features from projection-aware local state channels `diff = 0.5 * (phi - psi)`, balance residual `res = f - (phi + psi)`, normalized source `f`, and coefficient channels, then applies `(phi, psi) -> (phi + delta, psi - delta)` so `phi + psi` is unchanged before projection.
