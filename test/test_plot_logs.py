...
def test_parse_log_current_coupling_format_and_ignore_noise(tmp_path: Path) -> None:
    log_text = "\n".join(
        [
            ...
            "_run_training_phase - epoch 1 | train loss=9.4756e-03 l2_cons=6.0632e-03 flux_cons=3.4125e-01 cross_cons=7.5000e-02 rel_flux=1.0129e+00 rel_sol=1.5867e+00 | w_l2=1.0000e+00 on_l2=True mode_l2=manual w_flux=1.0000e-02 on_flux=True mode_flux=auto_operator w_cross=5.0000e-01 on_cross=False mode_cross=manual | lr=1.0000e-03 | val loss=1.0300e-02 l2_cons=6.7485e-03 flux_cons=3.5588e-01 cross_cons=8.2000e-02 rel_flux=1.0224e+00 rel_sol=1.5545e+00 | val_w_l2=1.0000e+00 val_w_flux=1.2000e-02 val_w_cross=0.0000e+00",
            "_run_training_phase - epoch 1 | train loss=8.0000e-03 l2_cons=5.0000e-03 flux_cons=3.0000e-01 cross_cons=4.0000e-02 rel_flux=9.0000e-01 rel_sol=1.4000e+00 | w_l2=1.0000e+00 on_l2=True mode_l2=manual w_flux=2.0000e-02 on_flux=False mode_flux=manual w_cross=2.5000e-01 on_cross=True mode_cross=auto_operator | lr=9.0000e-04 | val loss=4.0000e-03 l2_cons=4.0000e-03 flux_cons=2.0000e-01 cross_cons=1.0000e-01 rel_flux=8.0000e-01 rel_sol=1.2000e+00 | val_w_l2=1.0000e+00 val_w_flux=0.0000e+00 val_w_cross=2.0000e-01",
        ]
    )
    ...
    assert metrics["val_w_l2"] == [1.0000e00, 1.0000e00]
    assert metrics["val_w_flux"] == [1.2000e-02, 0.0000e00]
    assert metrics["val_w_cross"] == [0.0000e00, 2.0000e-01]
...
def test_plot_logs_figures_accept_current_metrics(tmp_path: Path) -> None:
    path = tmp_path / "training.log"
    path.write_text(
        "_run_training_phase - epoch 1 | train loss=1.0e-02 l2_cons=6.0e-03 flux_cons=4.0e-01 cross_cons=2.0e-01 rel_flux=1.0e+00 rel_sol=1.5e+00 | w_l2=1.0e+00 on_l2=True mode_l2=manual w_flux=1.0e-02 on_flux=True mode_flux=auto_operator w_cross=1.0e+00 on_cross=True mode_cross=manual | lr=1.0e-03 | val loss=1.3e-02 l2_cons=7.0e-03 flux_cons=3.0e-01 cross_cons=3.0e-01 rel_flux=1.1e+00 rel_sol=1.6e+00 | val_w_l2=1.0e+00 val_w_flux=2.0e-02 val_w_cross=1.0e+00\n"
    )
