...
def test_parse_log_current_coupling_format_and_ignore_noise(tmp_path: Path) -> None:
    log_text = "\n".join(
        [
            ...
            "_run_training_phase - epoch 1 | train loss=9.4756e-03 cons=6.0632e-03 flux_cons=3.4125e-01 rel_flux=1.0129e+00 rel_sol=1.5867e+00 | lam_flux=1.0000e-02 flux_on=True | lr=1.0000e-03 | val cons=6.7485e-03 flux_cons=3.5588e-01 rel_flux=1.0224e+00 rel_sol=1.5545e+00",
            "_run_training_phase - epoch 1 | train loss=8.0000e-03 cons=5.0000e-03 flux_cons=3.0000e-01 rel_flux=9.0000e-01 rel_sol=1.4000e+00 | lam_flux=2.0000e-02 flux_on=False | lr=9.0000e-04 | val cons=4.0000e-03 flux_cons=2.0000e-01 rel_flux=8.0000e-01 rel_sol=1.2000e+00",
        ]
    )
    ...
    assert metrics["loss_val"][0] == 6.7485e-03 + 1.0000e-02 * 3.5588e-01
    assert metrics["loss_val"][1] == 4.0000e-03


def test_plot_logs_figures_accept_current_metrics(tmp_path: Path) -> None:
    path = tmp_path / "training.log"
    path.write_text(
        "_run_training_phase - epoch 1 | train loss=1.0e-02 cons=6.0e-03 flux_cons=4.0e-01 rel_flux=1.0e+00 rel_sol=1.5e+00 | lam_flux=1.0e-02 flux_on=True | lr=1.0e-03 | val cons=7.0e-03 flux_cons=3.0e-01 rel_flux=1.1e+00 rel_sol=1.6e+00\n"
    )
