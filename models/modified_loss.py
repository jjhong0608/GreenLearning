...
class Loss:
    ...
    def __call__(
        self, green_output: torch.Tensor, u: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        """
        ...
        green_output : torch.Tensor
            ...
            Shape: (number_of_singular_points * number_of_points, 1).
        ...
        residual = u - green_output - self.hom_sol
        ...
        #     torch.sum(residual.square(), dim=0) / torch.sum(u.square(), dim=0)
        # )
        mean_error = torch.mean(torch.sum(residual**2, dim=0)) / u.shape[0]
