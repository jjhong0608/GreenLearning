...
from .MLP import MLP
from .loss_no_homogeneous import Loss
...
class GreenLearning:
    ...
    def load_data(self, path: Union[str, Path]) -> "GreenLearning":
        ...
        bd1 = torch.stack(
            torch.meshgrid(
                torch.tensor([self.x[0], self.x[-1]]).to(self.device),
                self.y,
                indexing="ij",
            ),
            dim=2,
        ...
        bd2 = torch.stack(
            torch.meshgrid(
                self.x,
                torch.tensor([self.y[0], self.y[-1]]).to(self.device),
                indexing="ij",
            ),
            dim=2,
        ).reshape(-1, 2)
        self.boundary_input = torch.cat([bd1, bd2], dim=0).to(self.device)
        print(f"boundary_input: {self.boundary_input.shape}\n{self.boundary_input}")
    ...
    def _initialize_optimizers(self) -> "GreenLearning":
        ...
        # self.adam = optim.Adam(self.green_network.parameters(), lr=1e-3)
        self.lars = LARS(
    ...
    def train(self) -> "GreenLearning":
        ...
        with Progress(
            ...
            for i in range(1, self.adam_epochs + 1):
                ...
                # bd_output = self.green_network(self.boundary_input)

                # Calculate the loss between predicted and actual solutions.
                # loss = criterion(green_output) + 5 * nn.MSELoss()(
                #     bd_output, torch.zeros_like(bd_output)
                # )
                loss = criterion(green_output)
    ...
    def plot_green_function(self, path: Optional[Union[str, Path]] = None) -> None:
        ...
        th_x = torch.as_tensor(x, device=self.device)
        th_y = torch.as_tensor(y, device=self.device)
        _input = (
            torch.stack(
        ...
        green_output = green_output.detach().cpu().numpy().T
        exact_green = exact_green.T
