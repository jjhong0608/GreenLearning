...
::: {.notes}
Explain only the two built-in analytic mechanisms. A(t)G0 supplies the Dirac delta jump. J0 is an antiderivative of G0, so S produces the derivative structure needed for Heaviside cancellation. Keep A and B definitions for Backup A if asked.
:::
...
::: {.notes}
End with the solver decomposition, not implementation. The reference solution is not used to train CouplingNet; it is used only for evaluation. More precisely, the CouplingNet optimization loss is split-energy consistency after projection and Green reconstruction.
:::
