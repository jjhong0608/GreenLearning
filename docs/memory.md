...
- `annulus_CDR/coupling8`과 `coupling9`은 fixed `eta=0.01`과 sample-wise
  closed-loop exact line search (`eta_cap=0.01`)를 제외한 saved config가 같다.
  Best validation canonical-energy epoch는 각각 46과 56이다. Best-energy
  checkpoint의 동일 50-sample test 비교에서 coupling9은 coupling8 대비 response
  mismatch `-0.735%`, canonical energy `+0.680%`, `rel_sol`
  `2.970490% -> 2.998864%`, split-transition jump RMS `+3.532%`, `rel_flux`
  `13.676697% -> 13.297429%`다. Flux 개선은 regular zone `-3.405%`에서 나오며
  transition flux는 `+1.527%` 악화된다. Coupling9의 mean `eta_star=0.011414`이고
  68% sample이 cap에 걸린다. 같은 frozen output에서 closed-loop는 fixed보다
  response mismatch를 낮추지만 canonical energy와 transition jump를 높이므로,
...
- Complex v6 training은 unweighted canonical audit와 optimized split energy를
  구분한다. Residual `r=u_phi-u_psi`에 대해 모든 same-segment `x_edges`와
  `y_edges`의 diffusion face energy를 물리 spacing과 `hx*hy` area weight로 합하고,
  모든 connected-segment endpoint의 general boundary energy를 별도로 계산한다.
  `loss_energy_consistency=E_bulk+E_boundary`는 항상 기록되는 audit이고,
  `loss_energy_optimized=E_bulk+boundary_weight*E_boundary`가 실제 base split
  objective와 best-energy checkpoint metric이다. Default `boundary_weight=1.0`은
  기존 canonical objective를 정확히 보존한다. Regular/transition 분류,
  length-jump score, group normalization은 production objective에서 사용하지 않는다.
- `coupling_training.canonical_energy.boundary_weight=0.0`은 complex-only
  boundary-off ablation이다. Boundary total/x/y는 계속 계산하고 evaluator,
  metric CSV, artifact에 기록하지만 gradient와 best-energy model selection에서는
  제외한다. Fixed sample-independent scalar이고 learnable weight/scheduler가 아니다.
  `configs/complex_coupling_soap_tangent_boundary_off.json`은 tangent baseline과 이
  설정 한 필드만 다른 paired config다. 이는 tangent correction과 endpoint anchor의
  역할 중복을 검증하기 위한 ablation이며 production default를 폐기한 것이 아니다.
- Complex v6의 `relative_split_consistency`는 opt-in이고 dataclass 기본값은
  disabled이다. Enabled이면 sample별 optimized energy를 사용해
  `(E_optimized + mass_weight*D_ref^-2*h_x*h_y*sum((u_phi-u_psi)^2)) /
...
- Canonical `configs/complex_coupling.json`은 energy-only v6 실험만 표현한다.
  ...
  처음부터 학습한다.
- Complex canonical audit energy는 valid-point physical bulk edge energy와 general
  ...
  `loss_energy_consistency`는 이 unweighted bulk+boundary 합이고,
  `loss_energy_optimized`만 configured boundary weight를 적용한다. 별도
  length-balanced metric은 없다.
...
- Complex v6는 reference-free checkpoint selection을 강제한다.
  ...
  `best_energy_checkpoint`는 validation `loss_energy_optimized`가 가장 작은
...
- The Annulus transition meeting deck now has 18 main slides plus two backups.
  Slide 16 defines the evaluated fixed tangent correction with `eta=0.01`,
  `lambda_rel=0.01`, and
  `rho_b=||m_post,b||_M/||m_pre,b||_M`. Slides 17-18 use frozen artifact-only
  evidence from `Annulus_poisson/coupling18` q50 sample 41 and
  `annulus_CDR/coupling8` q50 sample 33. Across all 50 test samples, mean
  response mismatch decreases by `65.0%` for Poisson and `63.0%` for CDR, with
  `50/50` samples improved in both. These percentages describe the tangent
  response mismatch, not solution-error reduction. Equal-mean/weak-final
  `rel_sol` and `rel_flux` remain evaluation metrics, and the deck makes no
