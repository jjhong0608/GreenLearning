...
::: {.notes}
This GreenNet-only evidence appears before CouplingNet is introduced. First show the reference and learned Green kernels, then use the signed error and diagnostics to argue that the learned kernel captures the singular Green structure for a reaction-free convection-diffusion line problem.
:::
...
::: {.notes}
The second state enlarges the fixed-η slice. Keep the signed-error heatmap visible so the audience can connect the one-dimensional slice accuracy with the two-dimensional kernel error pattern.
:::
...
::: {.notes}
End the GreenNet block with the takeaway, then transition to CouplingNet: after validating the line-wise Green inverse, the remaining question is how to split the 2D source so that the line-wise inverses form a consistent two-dimensional solution.
:::
...
::: {.notes}
Columns are selected by relative solution error quantiles from the convection-diffusion-reaction disk run: min, q25, q50, q75, and max.  The source row is visible first; reference, prediction, and signed-error rows reveal one row at a time.  The panels are intentionally title-free and axis-free; row and column labels live in the slide so the fields stay readable.  Source fields use per-sample scales.  Within each column, reference and prediction share one solution scale, while different columns use independent solution scales.  The signed-error row uses one zero-centered diverging scale across the selected samples, preserving error-magnitude comparison.  The metric card is visible from the start and labels the column quantity as relative solution error.  GreenNet evidence remains a reaction-free convection-diffusion kernel validation because the reference axial Green kernel is available there; this slide is solver-level CDR evidence for CouplingNet.
:::
