...
<p class="muted">Structure-preserving MOR framing</p>
:::
:::
:::
...
::: {.notes}
This slide answers two questions: why is this model reduction, and what exactly is being reduced? Keep coefficient variation out of the sample narrative; source variation is the sample variation.
:::
...
::: {.graphic-stage .ga-slices .fragment data-fragment-index="1"}
...
::: {.graphic-stage .ga-solution .fragment data-fragment-index="3"}
...
<p>Provide the source singularity.</p>
:::

::: {.role-pill .cancel}
<p class="label">Heaviside cancellation</p>
<p>Cancel the Heaviside contribution generated by the Dirac-delta jump.</p>
:::

::: {.role-pill .learned}
<p class="label">Learned smooth correction</p>
<p>Learn the remaining smooth residual.</p>
:::
:::
...
::: {.notes}
Start with the structural thesis only. The audience should first see the three roles before seeing the full formula.
:::
...
::: {.term-card .factor-card}
<p class="label">Coefficient factors</p>
...
::: {.notes}
Explain the two built-in analytic mechanisms. A(t)G0 supplies the Dirac delta jump. J0 is an antiderivative of G0, so S produces the derivative structure needed for Heaviside cancellation. Do not derive the full proof.
:::
...
::: {.notes}
End this slide by saying that the neural network is not responsible for inventing the Green function singularity. It only learns the smoother residual after the Dirac delta and Heaviside terms are built in.
:::
...
::: {.notes}
The important distinction is that the label is not the exact kernel value. The supervision is reconstruction of a generated target solution from a source generated from that target solution.
:::
...
::: {.fragment}
<p class="takeaway-line compact-takeaway">CouplingNet learns the directional source split that couples the axial Green reconstructions.</p>
:::

::: {.notes}
This is the transition from GreenNet to CouplingNet. The exact split would use the reference solution, but the slide writes \(u\) to keep notation light. Do not discuss branch internals yet; those belong on Slide 8.
:::
...
::: {.notes}
Keep the slide conceptual: multiple branch nets process source, coefficient profiles, and line-geometry structure, while trunk nets process pointwise coordinates. Avoid the phrase "fixed operator context" on the visible slide. Explain that coefficient profiles vary across axial lines even when the coefficient problem is fixed. Emphasize why pointwise transverse information belongs in a trunk: it is coordinate-dependent.
:::
...
<div class="coupling-evidence-layout">
  ...
  <div class="coupling-metric-card">
    ...
    <p class="metric-note">Source uses per-sample scales. Within each sample, reference and prediction share one solution scale; signed error uses a zero-centered shared scale.</p>
  </div>
</div>
```
...
Predict &phi;/&psi; so line-wise inverses form a 2D solution.
:::
...
<p>Axial Green operators provide the line-wise inverses,<br>and CouplingNet supplies the missing source coupling.</p>
:::

::: {.notes}
End with the solver decomposition, not implementation. This is the final statement before Q&A.
:::
...
::: {.term-card .learned}
<p class="label">Smooth residual</p>
...
::: {.two-panel}
::: {.interval-box}
<p class="panel-title">Connected physical interval</p>
