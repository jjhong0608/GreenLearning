...
class EvalCouplingCLI:
    ...
    def _build_coupling_model_config(
        ...
        if fusion_raw is None:
            ...
            if fusion_cfg.type not in EvalCouplingCLI._COUPLING_FUSION_TYPES:
                raise TypeError(
                    "coupling_model.fusion.type must be one of "
                    f"{sorted(EvalCouplingCLI._COUPLING_FUSION_TYPES)}."
                )
        else:
    ...
    def _build_coupling_losses_config(
        ...
        for key in ("l2_consistency", "flux_consistency", "cross_consistency"):
            ...
            if raw_term is None:
                ...
                parsed[key] = EvalCouplingCLI._build_coupling_loss_term_config(
    ...
    def _build_coupling_loss_term_config(
        ...
        if term_cfg.weight_mode not in EvalCouplingCLI._COUPLING_WEIGHT_MODES:
            raise TypeError(
                f"{section_name}.weight_mode must be one of "
                f"{sorted(EvalCouplingCLI._COUPLING_WEIGHT_MODES)}."
            )
        return term_cfg
