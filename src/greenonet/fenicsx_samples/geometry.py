...
class RawComplexGeometryGrid:
    """Raw full-grid metadata needed to write Coupling sample NPZ files."""
...
class GeometryGridLoader:
    """Load raw geometry NPZ metadata while preserving full-grid information."""
    ...
    def load(self, path: Path | str) -> RawComplexGeometryGrid:
        ...
        with np.load(geometry_path, allow_pickle=False) as raw:
            ...
            if missing:
                raise KeyError(
                    "FEniCSx sample generation requires geometry NPZ keys: "
                    f"{', '.join(missing)}."
                )
        ...
        # Reuse the runtime validator so this generator cannot create samples for an
        # invalid complex geometry contract.
